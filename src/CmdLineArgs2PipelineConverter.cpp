...
CmdLineArgs2PipelineConverter::getFilterProducer(const std::string& filterName) const
{
    ...
    const std::vector<FilterDescriptor>& descriptors) const
{
    ...
    {
        const FilterProducer PRODUCER = getFilterProducer(descriptor.filterName);
        if (PRODUCER == nullptr)
            throw std::invalid_argument("Unknown filter: " + descriptor.filterName);
