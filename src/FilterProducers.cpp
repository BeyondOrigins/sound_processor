...
void requireArgs(const FilterDescriptor& descriptor, size_t expected)
{
    if(descriptor.args.size() != expected)
        throw std::invalid_argument("Filter '" + descriptor.filterName +
...
FilterProducers::timeStretchFilterCreator(const FilterDescriptor& descriptor)
{
    ...
    requireArgs(descriptor, 1);
    return new TimeStretchFilter(
        toDouble(descriptor.args[0], "timestretch factor"));
}
...
IFilter*
FilterProducers::generatorFilterCreator(const FilterDescriptor& descriptor)
{
